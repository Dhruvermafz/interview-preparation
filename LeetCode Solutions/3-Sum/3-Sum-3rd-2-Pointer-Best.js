...
var threeSumTwoPointer = function (nums) {
  ...
  for (let indexA = 0; indexA < nums.length - 2; indexA++) {
    ...
    // now check if sum is zero and if NOT, the run the next set of 2 if loop to update indexB and indexC
    while (indexB < indexC) {
      const b = nums[indexB];
      const c = nums[indexC];
      ...
      // now with the below 2 if functions, just implementing how the indexB and indexC will be incremented and decremented with each iteration and gets feeded back to the above while function ( while (indexB < indexC ))
