  ...
  if (errors.length > 0) {
    ...
    User.findOne({ email: email }).then((user) => {
      if (user) {
        ...
        const mailOptions = {
          from: '"Auth Admin" <your-email@gmail.com>', // sender address
          ...
          if (user) {
            ...
            bcryptjs.genSalt(10, (err, salt) => {
              bcryptjs.hash(newUser.password, salt, (err, hash) => {
                if (err) throw err;
                newUser.password = hash; // Fixed typo here
                newUser
                  .save()
                  .then((user) => {
                    req.flash(
  ...
  if (errors.length > 0) {
    ...
    User.findOne({ email: email }).then((user) => {
      if (!user) {
        ...
        const CLIENT_URL = "http://" + req.headers.host;
        const output = `
                <h2>Please click on the link below to reset your account password</h2>
  ...
  //------------ Checking password mismatch ------------//
    ...
    bcryptjs.genSalt(10, (err, salt) => {
      bcryptjs.hash(password, salt, (err, hash) => {
        ...
        User.findByIdAndUpdate(id, { password }, function (err, result) {
          if (err) {
