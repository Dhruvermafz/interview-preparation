...
const BubbleSort = () => {
  ...
  const solve = () => {
    for (let i = values.length, timer = 0; i > 0; timer += i - 1, i--) {
      setTimeout(() => {
        // this is setTimeout function it used to schedule innermost for loop ;
        for (let j = 1; j < i; j++) {
          setTimeout(() => {
            // this is setTimeout function it used to compare too elements ;
            ...
            setTimeout(() => {
              // this is setTimeout function it used to regain the original color  ;
              document.getElementById(
                ids[j]
